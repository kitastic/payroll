import string
import re
import datetime
import os
import PySimpleGUI as sg
from pprint import PrettyPrinter
import matplotlib
from matplotlib.figure import Figure
from matplotlib.backends.backend_tkagg import (
    FigureCanvasTkAgg, NavigationToolbar2Tk)
import Ai

matplotlib.use('TkAgg')


class View:
    def __init__(self):
        self.gui = None
        self.values = dict()
        self.event = ''
        self.activeSalonNames = []  # all, upscale, or posh
        self.allSalonNames = []
        self.startDate = ''
        self.endDate = ''
        self.dates = False
        self.btnColor = '#40444b'
        self.sbColor = '#91C0F6'

        self.eTab_lb_Emps = dict()
        self.mTab_lb_Emps = dict()
        self.mTab_empStatus = dict()
        self.ai = Ai.Ai()
        self.j = PrettyPrinter(
            indent=2,
            width=100,
            compact=True,
            sort_dicts=False,
        )
        self.__setup__()
        self.openApp()

    def __setup__(self):
        sg.theme('darkgrey9')
        menu_def = [['File', ['Load Settings', '-------------', 'Exit']],
                    # ['Payroll', ['Print Payroll', ['All::printpayroll'],
                    #              'Print Payroll Skip Webscrape', ['All::printpayrollskipwebscrape']]],
                    ['Payroll', ['Export Payroll Skip Webscrape', ['All::printpayrollskipwebscrape']]],
                    ['Import', ['Excel Sales::importExcel', 'Excel Modified Sales::importModifiedExcel']],
                    ['Reporting']
                    ]
        self.activeSalonNames = self.ai.getActiveSalonNames()
        self.allSalonNames = self.ai.getAllSalonNames()
        for s in self.activeSalonNames:
            salon = s.capitalize()
            # menu_def[1][1][1].append(salon + '::printpayroll')
            menu_def[1][1][1].append(salon + '::printpayrollskipwebscrape')

        logTab = [[sg.Multiline(font='Courier 10', expand_x=True, expand_y=True, write_only=True,
                                reroute_stdout=True, reroute_stderr=True, echo_stdout_stderr=True, autoscroll=True,
                                auto_refresh=True)]]
        # ---------------------------------------------------------------------
        # reports tab
        # ---------------------------------------------------------------------
        rtabSalons = self.activeSalonNames.copy()
        rtabSalons.insert(0, 'all')
        rTab_r1 = sg.Frame('', [
            [sg.Column([
                [sg.Image('../images/shop-30.png', expand_x=True)],
                [sg.OptionMenu(values=rtabSalons, default_value='all', size=10, key='-rTab_om_salon-')],
            ]),
                sg.Column([
                    [sg.Button('', image_filename='../images/calendar20.png', button_color='#40444b',
                               key='-rTab_cal_sDate-'),
                     sg.Input('1/1/2021', key='-rTab_in_sDate-', size=15)],
                    [sg.Button('', image_filename='../images/calendar20.png', button_color='#40444b',
                               key='-rTab_cal_eDate-'),
                     sg.Input('12/31/2021', key='-rTab_in_eDate-', size=15)],
                ]),
                sg.Column([
                    [sg.T('Frequency')],
                    [sg.OptionMenu(values=(['Daily', 'Weekly', 'Monthly', 'Yearly']), default_value='monthly',
                                   size=10, key='-rTab_om_frequency-')],
                ]),
                sg.Column([
                    [sg.Checkbox('Self-comparison', default=False, key='-rTab_cb_selfcompare-')],
                ]),
                sg.Column([
                    [sg.Button(image_filename='../images/refresh24.png', key='-rTab_btn_load-')],
                ])
            ]])
        rTab_r2 = sg.Frame('', [[sg.Canvas(key='-rTab_canvas-')]], expand_x=True, expand_y=True)
        reportsTab = [[rTab_r1], [rTab_r2]]

        # ---------------------------------------------------------------------
        # salon tab
        # ---------------------------------------------------------------------
        sTabLeftFrame = sg.Frame('', [
            [sg.T('Current Salons:'), sg.Combo(values=self.allSalonNames, key='-sTab_c_salon-', expand_x=True),
             sg.Button(image_filename='../images/refresh24.png', key='-sTab_btn_load-')],
            [sg.T('')],
            [sg.HorizontalSeparator()],
            [sg.Image(filename='../images/password-40.png', )],
            [sg.T('Salon Name:'), sg.Input('', key='-sTab_in_name-')],
            [sg.T('Username:'), sg.Input('', key='-sTab_in_uname-')],
            [sg.T('Password:'), sg.Input('', key='-sTab_in_pass-')],
            [sg.Checkbox('Active', key='-sTab_cb_active-')],
            [sg.T('')],
            [sg.Frame('', [
                [sg.Column([
                    [sg.T('Save')],
                    [sg.Button(image_filename='../images/add-fingerprint-40.png', tooltip='Save salon',
                               key='-sTab_btn_save-')]
                ]),
                    sg.Column([
                        [sg.T('Update')],
                        [sg.Button(image_filename='../images/save-as-40.png',
                                   key='-sTab_btn_update-', tooltip='Update Salon')],
                    ]),
                    sg.Column([
                        [sg.T('Clear')],
                        [sg.Button(image_filename='../images/erase-40.png', key='-sTab_btn_clear-',
                                   tooltip='Clear form')]
                    ]),
                    sg.Column([
                        [sg.T('Remove')],
                        [sg.Button(image_filename='../images/remove-fingerprint-40.png', key='-sTab_btn_remove-',
                                   tooltip='Remove salon')],
                    ])
                ]
            ], expand_x=True, vertical_alignment='bottom', element_justification='c')]
        ], expand_y=True, size=(300))
        sTab = [
            [sTabLeftFrame,
             sg.Frame('', [
                 [sg.Image(filename='../images/shop-40.png', expand_x=True)],
                 [sg.Multiline('', key='-sTab_in_display-', horizontal_scroll=True, autoscroll=True, auto_refresh=True,
                               expand_x=True, expand_y=True)]], expand_x=True, expand_y=True)]
        ]

        # ------------------------------------------------------------------------------------------
        # employee tab
        # ------------------------------------------------------------------------------------------
        paygradeFrame = [[sg.T('Role'),
                          sg.OptionMenu(values=['Regular', 'Cash', 'Checkdeal', 'Janitor', 'Owner'],
                                                key='-eTab_om_role-', ),
                          sg.Button(image_filename='../images/refresh-22.png', key='-eTab_btn_role-')],
                         [sg.HorizontalSeparator()],
                         [sg.T('Regular', font=('default', 12, 'italic'))],
                         [sg.Text('Commission:'.ljust(20)),
                          sg.Combo((0.5, 0.6, 0.7), default_value=0, key='-eTab_c_commission-', size=(5, 1),
                                   disabled=True),
                          sg.Text('Check:'.ljust(20)),
                          sg.Combo((0.5, 0.6, 0.7), default_value=0, key='-eTab_c_check-', size=(5, 1), disabled=True),
                          ],
                         [sg.HorizontalSeparator()],
                         [sg.T('Special', font=('default', 12, 'italic'))],
                         [sg.Text('Commission:'.ljust(20)),
                          sg.Combo((0.5, 0.6, 0.7), default_value=0, key='-eTab_c_commissionspecial-', size=(5, 1),
                                   disabled=True),
                          sg.T('Cash Rate:'.ljust(18), expand_x=False),
                          sg.Combo((1, 0.85, 0.80, 0.75), default_value=0, key='-eTab_c_cashrate-', size=(5, 1),
                                   disabled=True),
                          ],
                         [sg.T('Check Original:'.ljust(21), expand_x=False),
                          sg.Combo((0.5, 0.6, 0.7), default_value=0, key='-eTab_c_checkoriginal-', size=(5, 1),
                                   disabled=True, ),
                          sg.T('Check Deal:'.ljust(17), expand_x=False),
                          sg.Combo((0.5, 0.6, 0.7), default_value=0, key='-eTab_c_checkdeal-', size=(5, 1),
                                   disabled=True),
                          ],
                         [sg.Checkbox('Print Checks', default=True, key='-eTab_cb_printchecks-')],
                         ]

        empTabLeftCol = sg.Frame('', [
            [sg.Text('Salon: '),
             sg.OptionMenu(values=self.activeSalonNames, key='-eTab_om_salon2-')
             ],
            [sg.T('Status'),
             sg.Button(image_data=toggle_btn_on, key='-eTab_btn_status-', image_subsample=2, border_width=0,
                       button_color=(sg.theme_background_color(), sg.theme_background_color()),
                       metadata=BtnInfo())],
            [sg.Text('Name:'), sg.Input(key='-eTab_in_empName-', expand_x=True)],
            [sg.Text('Name on check:'), sg.Input(key='-eTab_in_empCheckName-', expand_x=True)],
            [sg.Text(f'{"Pay 6 Day:":<16}'), sg.Input(key='-eTab_in_basePay6-', size=10),
             sg.Text(f'{"Pay 7th Day:":<15}'), sg.Input(key='-eTab_in_basePay7-', size=10)],
            [sg.Text(f'{"Rent/Week:":<15}'), sg.Input(key='-eTab_in_rent-', size=10),
             sg.Text(f'{"Fees/Day:":<16}'), sg.Input(key='-eTab_in_fees-', size=10)],
            [sg.Text('Check Bonus:'.ljust(14)), sg.Input(key='-eTab_in_checkbonus-', size=10),
             sg.Text('Check Fee:'.ljust(15)), sg.Input(key='-eTab_in_checkfee-', size=10)],
            [sg.Text('Cash Bonus:'.ljust(15)), sg.Input(key='-eTab_in_cashbonus-', size=10),
             sg.Text('Cash Fee:'.ljust(15)), sg.Input(key='-eTab_in_cashfee-', size=10)],
            [sg.Checkbox('Custom Booth', key='-eTab_cb_booth-'), sg.Input(key='-eTab_in_boothcustom-')],
            [sg.Frame('Pay Type', paygradeFrame, font=('bold'),expand_x=True)],
            [sg.T('Work Days (if Janitor)')],
            [sg.Checkbox('M', key='-eTab_janitor_mon-', default=True),
             sg.Checkbox('T', key='-eTab_janitor_tue-', default=True),
             sg.Checkbox('W', key='-eTab_janitor_wed-', default=True),
             sg.Checkbox('Th', key='-eTab_janitor_thu-', default=True),
             sg.Checkbox('F', key='-eTab_janitor_fri-', default=True),
             sg.Checkbox('Sat', key='-eTab_janitor_sat-', default=True),
             sg.Checkbox('Sun', key='-eTab_janitor_sun-', default=True)],
            [sg.Frame('', [
                [sg.Column([
                    [sg.T('Save New')],
                    [sg.Button(image_filename='../images/add-40.png', tooltip='Save New Employee',
                               key='-eTab_btn_save-')]
                ]),
                    sg.Column([
                        [sg.T('Clear Form')],
                        [sg.Button(image_filename='../images/erase-40.png', key='-eTab_btn_clear-',
                                   tooltip='Clear Form')]
                    ]),
                    sg.Column([
                        [sg.T('Update Existing')],
                        [sg.Button(image_filename='../images/save-as-40.png',
                                   key='-eTab_btn_update-', tooltip='Update Employee')],
                    ])
                ]
            ], expand_x=True)]
        ], expand_x=True, expand_y=True)

        empTabRightCol = sg.Frame('', [
            [sg.T('Choose Salon '),
             sg.OptionMenu(values=self.activeSalonNames, key='-eTab_om_salon-'),
             sg.Button(image_filename='../images/refresh24.png', key='-eTab_btn_load-'),
             sg.Checkbox('Show Active Only', key='-eTab_c_status-', default=True)],
            [sg.Image('../images/staff50.png', expand_x=True, )],
            [sg.Listbox(values=[], select_mode='extended', size=(50, 22), key='-eTab_lb-', enable_events=True,
                        expand_y=True)],
            [sg.Column([[sg.Button(key='-eTab_btn_remove-', image_filename='../images/trash-40.png',
                                   tooltip='Delete employee')]], justification='r')]
        ], expand_x=True, expand_y=True)

        empTab = [[sg.Column([[empTabLeftCol]], expand_y=True, ), sg.Column([[empTabRightCol]], expand_y=True, )]]

        # ------------------------------------------------------------------------------------------
        # main tab
        # ------------------------------------------------------------------------------------------
        # button menu items based on salon names given and not hardcoded
        bm1 = [i[0].lower() for i in self.activeSalonNames]
        bm1.insert(0, 'a')
        bm2 = [i for i in self.activeSalonNames]
        bm2.insert(0, 'All')

        menuIconDates = sg.Frame('Payroll Date Range',
                                 [[sg.Button('', image_filename='../images/calendar20.png', button_color='#40444b',
                                             key='-main_cal_sDate-'),
                                   sg.Input('Enter Start Date', key='-main_in_sDate-', size=15)],
                                  [sg.Button('', image_filename='../images/calendar20.png', button_color='#40444b',
                                             key='-main_cal_eDate-'),
                                   sg.Input('Enter End Date', key='-main_in_eDate-', size=15), ],
                                  ], size=(150, 75))
        mTab_r1c2 = sg.Column([
                              [sg.T("Export \nw/o web")],
                              [sg.ButtonMenu('', [bm1, bm2], tooltip='Export payroll without webscrape',
                               image_filename='../images/web-none-50.png', key='-mTab_btn_export_wo_web-',
                               button_color=self.btnColor)]
        ])
        mTab_r1c3 = sg.Column([[sg.T('View\nPayroll')],
                               [sg.Button('', image_filename='../images/money-transfer-45.png',
                                          key='-mTab_btn_payroll-')],
                               ])
        mTab_r1c4 = sg.Column([[sg.T('View\nStatus')],
                               [sg.Button(image_filename='../images/in-progress-45.png', key='-mTab_btn_status-',
                                          tooltip='Current week income status for employees')]
                               ])
        mTab_r1c5 = sg.Column([[menuIconDates]])
        mTab_r1c6 = sg.Column([[sg.Checkbox('This Week', default=False, key='-mTab_cb_thisweek-', enable_events=True)],
                               [sg.Checkbox('Gurantee', default=False, key='-guarantee-')],
                               [sg.T('Manual Booth Rent: '),
                                sg.Input('', size=(8, 1), tooltip='Enter dollar amount', key='-mTab_in_booth-')],
                               ])

        mainTab = [[mTab_r1c2, mTab_r1c3, mTab_r1c4, mTab_r1c5, mTab_r1c6],
                   [sg.Frame('', [[sg.T('Salon: '), sg.Combo(values=self.activeSalonNames, size=12,
                                                             key='-mTab_c_salon-', enable_events=True,
                                                             default_value='upscale'),
                                   sg.Button(image_filename='../images/refresh24.png', key='-mTab_btn_load-')],
                                  [sg.Push(), sg.T('Employees'), sg.Push()],
                                  [sg.Listbox(values=[], select_mode='extended', key='-mTab_lb-', expand_y=True,
                                              expand_x=True, enable_events=True)]], expand_y=True, size=(250, 550)),
                    sg.Frame('', [[sg.Multiline('', key='-mTab_in_display-', expand_x=True, expand_y=True,
                                                autoscroll=True, horizontal_scroll=True)]], expand_y=True,
                             expand_x=True)],
                   ]

        # ------------------------------------------------------------------------------------------
        # menu button bar (located beneath main menu bar)
        # ------------------------------------------------------------------------------------------
        dbIconDates = sg.Frame('Database Date Range',
                                 [[sg.Button('', image_filename='../images/calendar20.png', button_color='#40444b',
                                             key='-db_cal_sDate-'),
                                   sg.Input('Enter Start Date', key='-db_in_sDate-', size=15)],
                                  [sg.Button('', image_filename='../images/calendar20.png', button_color='#40444b',
                                             key='-db_cal_eDate-'),
                                   sg.Input('Enter End Date', key='-db_in_eDate-', size=15), ],
                                  ], size=(150, 75))
        self.layout = [[sg.Menubar(menu_def)],
                       [sg.Column([[sg.Image('../images/kp_w40.png')]]),
                        dbIconDates,
                        sg.Column([[sg.T("Sales\n", justification='center')],
                                   [sg.ButtonMenu('', [bm1, bm2], tooltip='Download sales',
                                                  image_filename='../images/database-50.png', key='getSales',
                                                  button_color=self.btnColor)]]),
                        sg.Column([[sg.T('Last Date', )], [sg.T('', key='jsonInfo')]]),
                        sg.Column([[sg.T('Modified\nSales')],
                                   [sg.ButtonMenu('', [bm1, bm2], tooltip='Download modified sales',
                                                  image_filename='../images/database-updated-50.png',
                                                  key='getModifiedSales',
                                                  button_color=self.btnColor)]]),
                        sg.Column([[sg.T('Last Date', )], [sg.T('', key='jsonModifiedInfo')]]),
                        sg.Column([[sg.T("", expand_x=True)]], expand_x=True),
                        sg.Column([[sg.Button(image_filename='../images/save-50.png', button_color='#40444b',
                                              key='-Save-'),
                                    sg.Button(image_filename='../images/shutdown-50.png', button_color='#40444b',
                                              key='Exit')
                                    ]]),
                        ], ]
        self.layout += [[sg.TabGroup([[sg.Tab('Main', mainTab),
                                       sg.Tab('Employees', empTab),
                                       sg.Tab('Salon', sTab),
                                       sg.Tab('Reports', reportsTab),
                                       sg.Tab('Log', logTab),
                                       ]], expand_x=True, expand_y=True)],
                        # key '-noticeBuffer-' is a MUST HAVE in order to make status bar show because
                        # it expands (assigned after starting window) and make sure there is space for the bar
                        [sg.T(key='-noticeBuffer-', font='ANY 1', pad=(0, 0))],
                        [sg.Multiline('', key='-notice-', size=(90, 5), expand_x=True, autoscroll=True)]]

    def openApp(self):
        # sg.show_debugger_window(location=(10,10))
        self.gui = sg.Window('', self.layout,
                             # size=(700,735),
                             resizable=True,
                             finalize=True,
                             sbar_background_color=self.sbColor,
                             button_color=self.btnColor,
                             location=(0, 25)
                             )
        self.gui.set_min_size(self.gui.size)
        # setup canvas for reports graphing
        fig = Figure(figsize=(8, 4), dpi=100, layout='tight', )
        ax = fig.subplots(sharex=True)
        tkcanvas = FigureCanvasTkAgg(fig, self.gui['-rTab_canvas-'].TKCanvas)
        toolbar = NavigationToolbar2Tk(tkcanvas, self.gui['-rTab_canvas-'].TKCanvas, pack_toolbar=True)
        toolbar.update()
        tkcanvas.get_tk_widget().pack(side='top', fill='both', expand=1)
        self.gui.set_min_size(self.gui.size)
        self.gui['jsonInfo'].update(self.ai.getJsonLatestDates('display', 'regular'))
        self.gui['jsonModifiedInfo'].update(self.ai.getJsonLatestDates('display', 'modified'))
        salonLists = ['-mTab_c_salon-', '-eTab_om_salon-', '-eTab_om_salon2-', '-sTab_c_salon-', ]
        for lists in salonLists:
            self.gui[lists].update(value=self.activeSalonNames[0])
        show_all = self.gui['-eTab_c_status-'].get()
        for s in self.activeSalonNames:
            self.eTab_lb_Emps[s] = self.ai.populateEmpList(s, show_all)

        # setup commands to clean up gui while True loop
        mBar = ['Load Settings', 'Excel Sales::importExcel', 'Excel Modified Sales::importModifiedExcel',
                'All::printpayroll', 'All::printpayrollskipwebscrape', 'Reporting']
        for s in self.activeSalonNames:
            s = s.capitalize()
            mBar.append(s + '::printpayroll')
            mBar.append(s + '::printpayrollskipwebscrape')

        # items in these lists should trigger an immediate action, can be left out if not
        mTab = ['-main_cal_sDate-', '-main_in_sDate-', '-main_cal_eDate-', '-main_in_eDate-',
                '-db_cal_sDate-', '-db_in_sDate-', '-db_cal_eDate-', '-db_in_eDate-',
                '-mTab_c_salon-', '-mTab_btn_load-', '-mTab_cb_thisweek-', '-mTab_btn_sales-',
                '-mTab_btn_payroll-', '-mTab_c_salon-', '-mTab_btn_export_wo_web-', '-mTab_btn_export_w_web-',
                '-mTab_btn_status-', '-mTab_lb-']

        eTab = ['-eTab_btn_role-', '-eTab_btn_status-', '-eTab_btn_save-',
                '-eTab_btn_clear-', '-eTab_btn_update-', '-eTab_btn_remove-', '-eTab_btn_load-',
                '-eTab_lb-', '-eTab_om_salon-', '-eTab_om_salon2-']

        sTab = ['-sTab_c_salon-', '-sTab_btn_load-',
                '-sTab_btn_save-', '-sTab_btn_update-', '-sTab_btn_clear-', '-sTab_btn_remove-',
                '-sTab_in_display-', ]

        rTab = ['-rTab_cal_sDate-', '-rTab_in_sDate-', '-rTab_cal_eDate-', '-rTab_in_sDate-', '-rTab_om_salon-',
                '-rTab_om_frequency-', '-rTab_btn_load-']

        while True:
            event, values = self.gui.read()
            if event not in (sg.TIMEOUT_EVENT, sg.WIN_CLOSED):
                self.gui['-notice-'].update(event)
                print('============ Event = ', event, ' ==============')
                print('-------- Values Dictionary (key=value) --------')
                for key in values:
                    print(key, ' = ', values[key])
            if event == "Exit" or event == sg.WIN_CLOSED:
                break

            self.values = self.values | values
            self.event = event
            if event in mTab:
                self.listenMTab()
            elif event in mBar:
                self.listenMBar()
            elif event in eTab:
                self.listenETab()
            elif event in sTab:
                self.listenSTab()
            elif event in rTab:
                self.listenRTab(fig, ax)
            elif event in ['getSales', 'getModifiedSales']:
                start_date = self.values['-db_in_sDate-']
                end_date = self.values['-db_in_eDate-']
                self.verifyDates(sdate=start_date, edate=end_date)
                if not self.dates:
                    self.status('ERROR: choose correct dates from "Database Date Range"')
                    continue

                salesType = 'regular' if event == 'getSales' else 'modified'
                self.status(f'WORKING: webscraping {salesType} sales...')
                '''
                    Webscrape start date should be inclusive of recent json date in case
                    last information pull was mid-day and not a full day of record
                '''
                salon = self.values['getSales'] if event == 'getSales' else self.values['getModifiedSales']
                status, msg = self.ai.webscrapeSales(salon, start_date, end_date, salesType)
                if not status:
                    self.status(msg)
                    continue
                if event == 'getSales':
                    self.gui['jsonInfo'].update(self.ai.getJsonLatestDates('display', salesType))
                else:
                    self.gui['jsonModifiedInfo'].update(self.ai.getJsonLatestDates('display', salesType))
                self.status('SUCCESS: Finished webscraping and updating json database')

            elif event == '-Save-':
                self.status('WORKING: saving to database...')
                self.ai.saveNewSettings()
                self.status(f'SUCCESS: Finished saving to db')
            else:
                self.status('[View.openApp] command not found')

        self.exitProgram()

    def listenMBar(self):
        if self.event == 'Load Settings':
            self.status('WORKING: load settings...')
            pfname = sg.popup_get_file('Choose Json settings', 'Import Settings', os.getcwd())
            if pfname:
                if os.path.isfile(pfname):
                    self.ai.loadSettings(pfname)
                else:
                    self.ai.loadSettings(pfname=None)
                self.status('SUCCESS: loaded settings')
            else:
                self.status('ERROR: settings not loaded')
                pass

        elif '::printpayroll' in self.event:
            self.status('WORKING: print payroll...')
            webscrape = False if 'skipwebscrape' in self.event else True
            # last_modified_dates = self.ai.getJsonLatestDates('webscrape', 'modified')
            last_dates = self.ai.getJsonLatestDates('webscrape', 'regular')
            # today = datetime.datetime.today().strftime('%m/%d/%Y')
            salon_name = re.search(r'^\w+', self.event).group(0).lower()
            salon_marker = self.getMarker(salon_name)
            self.verifyDates()
            if not self.dates:
                self.status(f'[View.listenMBar] {salon_name} salon cannot determine dates')
                pass

            for wanted_salon, sname, start_date in zip(salon_marker, self.activeSalonNames, last_dates):
                status = False
                msg = False
                if not wanted_salon:
                    continue
                result, data = self.ai.getPayrollFromSalon(sname, self.startDate, self.endDate,
                                                           self.values['-guarantee-'],
                                                           self.values['-mTab_in_booth-'])
                if result:
                    status_export, msg = self.ai.exportPayroll(sname, self.startDate, self.endDate, 'html')
                    if not status_export:
                        self.status(msg)
                        return
                else:
                    self.status(f'ERROR: [View.listenMBar]>unable to get payroll from salon. [Salon.getPayroll]>{data}')
                    return

            self.gui['jsonInfo'].update(self.ai.getJsonLatestDates('display', 'regular'))
            self.status(f'SUCCESS: completed exporting files')

        elif self.event in ['Excel Sales::importExcel', 'Excel Modified Sales::importModifiedExcel']:
            layout = [[sg.T('Salon'), sg.Combo(values=self.activeSalonNames, key='sname')],
                      [sg.Input('', key='-salespfname-'),
                       sg.FileBrowse(target='-salespfname-', initial_folder=os.getcwd())],
                      [sg.Button('Submit'), sg.Button('Cancel')]]
            win = sg.Window('Choose excel sales file to import', layout, finalize=True)
            win.read()
            pfname = ''
            salon_name = ''
            while True:
                event, values = win.read()
                if event == sg.WIN_CLOSED or event == 'Cancel':
                    break
                elif event == 'Submit':
                    pfname = values['-salespfname-']
                    salon_name = values['sname']
                    break
            win.close()
            type = 'regular' if self.event == 'Excel Sales::importExcel' else 'modified'
            if pfname and salon_name:
                self.ai.importJson(salon_name, pfname, type)
            if type == 'regular':
                self.gui['jsonInfo'].update(self.ai.getJsonLatestDates('display', type))
            else:
                self.gui['jsonModifiedInfo'].update(self.ai.getJsonLatestDates('display', type))
            self.status(f'SUCCESS: Import completed')

    def listenMTab(self):
        if self.event in ['-main_cal_sDate-', '-db_cal_sDate-', '-main_cal_eDate-', '-db_cal_eDate-']:
            date = sg.popup_get_date(title='Choose date')
            if not date:
                self.status('END: (View.listenMTab) failed to get valid date')
                return

            sdate = '{}/{}/{}'.format(date[0], date[1], date[2])
            if self.event == '-db_cal_sDate-':
                self.gui['-db_in_sDate-'].update(sdate)
            elif self.event == '-db_cal_eDate-':
                self.gui['-db_in_eDate-'].update(sdate)
            elif self.event == '-main_cal_eDate-':
                self.gui['-main_cal_eDate-'].update(sdate)
            elif self.event == '-main_cal_sDate-':
                # tried simply adding 6 to day element and instead of going next month, day was 33
                # time delta is smarter and know when to increment month
                edate = datetime.datetime.strptime(sdate, '%m/%d/%Y') + datetime.timedelta(6)
                e = datetime.datetime.strftime(edate, '%m/%d/%Y')
                self.gui['-main_in_sDate-'].update(sdate)
                self.gui['-main_in_eDate-'].update(e)

        elif self.event == '-mTab_cb_thisweek-':
            self.verifyDates()

        elif self.event in ['-mTab_btn_export_wo_web-', '-mTab_btn_export_w_web-']:
            self.status('WORKING: exporting payroll...')
            salon_name = self.values['-mTab_btn_export_wo_web-'].lower() if self.event == '-mTab_btn_export_wo_web-' \
                else self.values['-mTab_btn_export_w_web-'].lower()
            self.verifyDates()
            if self.dates:
                result, data = self.ai.getPayrollFromSalon(salon_name, self.startDate, self.endDate,
                                                           self.values['-guarantee-'],
                                                           self.values['-mTab_in_booth-'])
                if result:
                    status_export, msg = self.ai.exportPayroll(salon_name, self.startDate, self.endDate, 'html')
                    if not status_export:
                        self.status(msg)
                else:
                    self.status(f'ERROR: {data}')
                self.gui['jsonInfo'].update(self.ai.getJsonLatestDates('display', 'regular'))
                self.status(f'SUCCESS: completed exporting files')
            else:
                self.status(f'ERROR: [{self.event}] {salon_name} salon cannot determine dates')

        elif self.event in ('-mTab_btn_payroll-', '-mTab_btn_status-', '-mTab_btn_load-'):
            self.verifyDates()
            sname = self.values['-mTab_c_salon-'].lower()
            if not self.dates and self.event != '-mTab_c_salon-':
                self.status("[View] Dates are not valid")
                return
            # it is better to perform payroll everytime any request is made because there may be other income
            # updates since last payroll command. ie such as updating json sales after performing payroll
            result, data = self.ai.getPayrollFromSalon(sname, self.startDate, self.endDate,
                                                       self.values['-guarantee-'],
                                                       self.values['-mTab_in_booth-'])
            if result:
                self.mTab_lb_Emps[sname] = data
                # this step performs '-mTab_btn_load-' event
                self.gui['-mTab_lb-'].update(data.keys())
            else:
                self.status(f'ERROR: [View.listenMTab]>unable to get payroll from salon. [Salon.getPayroll]>{data}')
                return

            if self.event == '-mTab_btn_status-':
                self.mTab_empStatus = self.ai.getEmpStatus(sname)
                tmp = ''
                for name, bundle in self.mTab_empStatus.items():
                    tmp += '{}:\n {}\n'.format(name, bundle)
                self.gui['-mTab_in_display-'].update(tmp)

        elif self.event == '-mTab_lb-':
            try:
                # make sure something is selected, will catch and error if nothing is selected
                salon = self.values['-mTab_c_salon-'].lower()
                n = self.values['-mTab_lb-'][0]
                e = self.mTab_lb_Emps[salon][n]
                self.gui['-mTab_in_display-'].update(e)
            except KeyError:
                pass
        # sDate = self.values['-main_in_sDate-']
        # eDate = self.values['-main_in_eDate-']
        # self.verifyDates(sDate, eDate)

    def listenETab(self):
        salonName = self.values['-eTab_om_salon-'].lower()
        if self.event == '-eTab_btn_load-':
            # send this command first to have a list to compare ids when adding new one
            self.refreshETabList(salonName)

        elif self.event in ['-eTab_btn_save-', '-eTab_btn_update-']:
            # grab employees and gather into dictionary to validate id and etc
            # this step is needed incase the first thing someone does is add employee without
            # populating list first. if not, the first thing throwing error is gathering all
            # ids to make avoid duplicates
            employee = self.parseEmp()
            if employee:
                task = ''
                result = ''
                msg = ''
                salonName = self.values['-eTab_om_salon2-']  # specified when creating employee, not from listbox
                if self.event == '-eTab_btn_save-':
                    task = 'save'
                    result, msg = self.ai.modEmp(task, salonName, employee)
                elif self.event == '-eTab_btn_update-':
                    task = 'update'
                    result, msg = self.ai.modEmp(task, salonName, employee)

                if result:
                    self.gui['-notice-'].update(f'Employee {task}d: {employee} in {salonName}')
                    self.refreshETabList(salonName)
                else:
                    self.gui['-notice-'].update(f'[View.listenETab] {task} error: {msg}\n'
                                                f'If updating, make sure employee exists first.')

        elif self.event == '-eTab_btn_remove-':
            try:
                eName = self.values['-eTab_lb-'][0]
                if eName:
                    if len(self.values['-eTab_lb-']) > 1:
                        for n in self.values['-eTab_lb-']:
                            self.ai.modEmp('remove', self.values['-eTab_om_salon-'].lower(), n)
                    else:
                        self.ai.modEmp('remove', self.values['-eTab_om_salon-'].lower(), eName)
                    self.refreshETabList(salonName)
                    self.gui['-notice-'].update(f'Employee removed: {eName} in {salonName}')
            except Exception:
                self.gui['-notice-'].update('ERROR: View.listenETab cannot remove employee > no name')

        elif '-eTab_btn_status-' in self.event:
            # this part changes the graphical button state
            self.gui[self.event].metadata.state = not self.gui[self.event].metadata.state
            self.gui[self.event].update(image_data=toggle_btn_on if self.gui[self.event].metadata.state else
                                        toggle_btn_off, image_subsample=2)
            # updates employee actual status
            salon = self.values['-eTab_om_salon-'].lower()
            name = self.values['-eTab_lb-'][0]
            e = self.eTab_lb_Emps[salon][name]
            e['active'] = self.gui[self.event].metadata.state

        elif self.event == '-eTab_btn_role-':
            self.togglePaygrade(self.values['-eTab_om_role-'])

        elif self.event == '-eTab_lb-':
            try:
                # make sure something is selected, will catch and error if nothing is selected
                salon = self.values['-eTab_om_salon-'].lower()
                n = self.values['-eTab_lb-'][0]
                e = self.eTab_lb_Emps[salon][n]
                self.empToGui(e)
            except KeyError:
                pass
        elif self.event == '-eTab_btn_clear-':
            self.clearBtn()

        else:
            self.gui['-notice-'].update('[View.listenEtab]: Command not found in employee tab')

    def listenSTab(self):
        if self.event == '-sTab_btn_save-':
            sname = self.values['-sTab_in_name-']
            uname = self.values['-sTab_in_uname-']
            password = self.values['-sTab_in_pass-']
            active = self.values['-sTab_cb_active-']
            # create salon first and then import json so that the default
            # pfname of json is still default
            salon = {'name': sname,
                     'login': {'username': uname, 'password': password},
                     'active': active,
                     'path': '../db/',  # might not need yet
                     'paymentsFnames': {},
                     'employees': {}}
            self.ai.createSalon(salon)
            self.activeSalonNames.append(sname)
            self.refreshSalonLists()
            self.gui['-notice-'].update(f'[View.listenSTab] save done')

        elif self.event == '-sTab_btn_update-':
            sname = self.values['-sTab_in_name-']
            uname = self.values['-sTab_in_uname-']
            password = self.values['-sTab_in_pass-']
            active = self.values['-sTab_cb_active-']
            # create salon first and then import json so that the default
            # pfname of json is still default
            salon = {'name': sname,
                     'login': {'username': uname, 'password': password}}
            self.ai.updateSalon(salon)
            self.refreshSalonLists()
            self.gui['-notice-'].update(f'[View.listenSTab] update done')

        elif self.event == '-sTab_btn_remove-':
            sname = self.values['-sTab_c_salon-']
            if sname in self.activeSalonNames:
                self.ai.removeSalon(sname)
                self.activeSalonNames.remove(sname)
                self.refreshSalonLists()
        elif self.event == '-sTab_btn_clear-':
            self.gui['-sTab_in_name-'].update('')
            self.gui['-sTab_in_uname-'].update('')
            self.gui['-sTab_in_pass-'].update('')
        elif self.event == '-sTab_btn_load-':
            salon = self.values['-sTab_c_salon-']
            login, niceprint = self.ai.getSalonInfo(salon)
            self.gui['-sTab_in_name-'].update(salon)
            self.gui['-sTab_in_uname-'].update(login['username'])
            self.gui['-sTab_in_pass-'].update(login['password'])
            self.gui['-sTab_cb_active-'].update(login['active'])
            self.gui['-sTab_in_display-'].update(niceprint)

    def listenRTab(self, fig, ax):
        if self.event == '-rTab_cal_sDate-':
            try:
                sDate = sg.popup_get_date(title='Choose start date')
                sdate = '{}/{}/{}'.format(sDate[0], sDate[1], sDate[2])
                self.gui['-rTab_in_sDate-'].update(sdate)
            except Exception:
                self.gui['-notice-'].update('ERROR:(View.listenRTab) failed to get valid date')
        elif self.event == '-rTab_cal_eDate-':
            try:
                eDate = sg.popup_get_date(title='Choose end date')
                edate = '{}/{}/{}'.format(eDate[0], eDate[1], eDate[2])
                self.gui['-rTab_in_eDate-'].update(edate)
            except Exception:
                self.gui['-notice-'].update('ERROR:(View.listenRTab) failed to get valid date')
        elif self.event == '-rTab_btn_load-':
            salon, sDate, eDate, frequency = ['' for i in range(4)]
            try:
                salon = self.values['-rTab_om_salon-']
                sDate = self.values['-rTab_in_sDate-']
                eDate = self.values['-rTab_in_eDate-']
                frequency = self.values['-rTab_om_frequency-']
            except Exception:
                self.gui['-notice-'].update('ERROR[View.listenRTab]: failed to load report')
                return
            if self.values['-rTab_cb_selfcompare-'] and salon == 'all':
                self.gui['-notice-'].update('[View.listenRTab.750]: cannot self-compare all salons')
                return
            if self.values['-rTab_cb_selfcompare-']:
                self.ai.graphCompare(salon, sDate, eDate, frequency, self.gui['-rTab_canvas-'], fig, ax)
            else:
                self.ai.graph(salon, sDate, eDate, frequency, self.gui['-rTab_canvas-'], fig, ax)

    def clearBtn(self):
        inputs = ['-eTab_in_salon-', '-eTab_in_empName-',
                  '-eTab_in_empCheckName-', '-eTab_in_basePay6-', '-eTab_in_basePay7-',
                  '-eTab_in_fees-', '-eTab_in_rent-',
                  '-eTab_in_checkbonus-', '-eTab_in_checkfee-',
                  '-eTab_in_cashbonus-', '-eTab_in_cashfee-',
                  '-eTab_in_booth-'
                  '-eTab_c_commission-', '-eTab_c_check-', '-eTab_c_commissionspecial-', '-eTab_c_checkdeal-',
                  '-eTab_c_checkoriginal-', '-eTab_om_role-'
                  ]
        checkboxes = ['-eTab_cb_booth-',
                      '-eTab_janitor_mon-', '-eTab_janitor_tue-',
                      '-eTab_janitor_wed-', '-eTab_janitor_thu-',
                      '-eTab_janitor_fri-', '-eTab_janitor_sat-', '-eTab_janitor_sun-']
        self.gui['-eTab_btn_status-'].metadata.setState(True)
        self.gui['-eTab_btn_status-'].update(image_data=toggle_btn_on, image_subsample=2)
        self.gui['-eTab_om_role-'].update(value='')
        for i in inputs:
            self.gui[i].update('')
        for i in checkboxes:
            self.gui[i].update(value=False)
        self.togglePaygrade(False)

    def empToGui(self, emp):
        """
            Updates the gui with information of employee retrieved from ai and fills
            the form on emloyees tab. When dealing with new fields added from gui and not yet in database,
            use dictionary.get(key, valueifkeyerror)
        Args:
            emp: employee dictionary
        Returns:
            None
        """
        if emp['active']:
            self.gui['-eTab_btn_status-'].metadata.setState(True)
            self.gui['-eTab_btn_status-'].update(image_data=toggle_btn_on, image_subsample=2)
        else:
            self.gui['-eTab_btn_status-'].metadata.setState(False)
            self.gui['-eTab_btn_status-'].update(image_data=toggle_btn_off, image_subsample=2)
        self.gui['-eTab_om_salon2-'].update(emp['salonName'])
        self.gui['-eTab_in_empName-'].update(emp['name'])
        self.gui['-eTab_in_empCheckName-'].update(emp['checkName'])
        self.gui['-eTab_in_basePay6-'].update(emp['pay6'])
        self.gui['-eTab_in_basePay7-'].update(emp['pay7'])
        self.gui['-eTab_in_fees-'].update(emp['fees'])
        self.gui['-eTab_in_rent-'].update(emp['rent'])
        self.gui['-eTab_in_checkbonus-'].update(emp.get('checkbonus', 0))
        self.gui['-eTab_in_checkfee-'].update(emp.get('checkfee', 0))
        self.gui['-eTab_in_cashbonus-'].update(emp.get('cashbonus', 0))
        self.gui['-eTab_in_cashfee-'].update(emp.get('cashfee', 0))
        self.gui['-eTab_in_boothcustom-'].update(emp.get('boothcustom', 0))
        self.gui['-eTab_cb_booth-'].update(emp.get('boothcustomflag', False))
        self.gui['-eTab_cb_printchecks-'].update(emp['printchecks'])
        self.gui['-eTab_om_role-'].update(value=emp['type']['role'].capitalize())
        if emp['type']['role'].lower() in ['regular', 'janitor', 'owner']:
            self.togglePaygrade(emp['type']['role'])
            self.gui['-eTab_c_commission-'].update(emp['type']['regular']['commission'])
            self.gui['-eTab_c_check-'].update(emp['type']['regular']['check'])
            self.gui['-eTab_c_commissionspecial-'].update(0)
            self.gui['-eTab_c_checkdeal-'].update(0)
            self.gui['-eTab_c_checkoriginal-'].update(0)
            self.gui['-eTab_c_cashrate-'].update(0)
        else:
            self.togglePaygrade(emp['type']['role'])
            self.gui['-eTab_c_commissionspecial-'].update(emp['type']['special']['commissionspecial'])
            self.gui['-eTab_c_checkdeal-'].update(emp['type']['special']['checkdeal'])
            self.gui['-eTab_c_checkoriginal-'].update(emp['type']['special']['checkoriginal'])
            self.gui['-eTab_c_cashrate-'].update(emp['type']['special']['cashrate'])
            self.gui['-eTab_c_commission-'].update(0)
            self.gui['-eTab_c_check-'].update(0)
        self.gui['-eTab_janitor_mon-'].update(value=emp['workdays']['mon'])
        self.gui['-eTab_janitor_tue-'].update(value=emp['workdays']['tue'])
        self.gui['-eTab_janitor_wed-'].update(value=emp['workdays']['wed'])
        self.gui['-eTab_janitor_thu-'].update(value=emp['workdays']['thu'])
        self.gui['-eTab_janitor_fri-'].update(value=emp['workdays']['fri'])
        self.gui['-eTab_janitor_sat-'].update(value=emp['workdays']['sat'])
        self.gui['-eTab_janitor_sun-'].update(value=emp['workdays']['sun'])

    def exitProgram(self):
        self.gui.close()

    def parseEmp(self):
        """
            instead of deciphering values everytime data passes between classes, this method
            will format it so that when employees object is passed around, it is easier to know
            what to do with it without having to catch all types of errors
        Returns:
            formatted employee dict
        """
        checkpoint = {'salonname': False, 'name': False, 'paygrade': False}
        currentSalon = self.values['-eTab_om_salon2-']
        status = self.gui['-eTab_btn_status-'].metadata.state

        # validating each field of information before sending to ai
        if self.values['-eTab_om_salon2-'] in self.activeSalonNames:
            checkpoint['salonname'] = True

        if len(self.values['-eTab_in_empName-']) >= 1:
            checkpoint['name'] = True

        checkName = self.values['-eTab_in_empCheckName-'].replace('  ', ' ')
        if len(checkName) >= 1:
            string.capwords(checkName)
        pay6 = self.test(self.values['-eTab_in_basePay6-'], 'float')
        pay7 = self.test(self.values['-eTab_in_basePay7-'], 'float')
        fees = self.test(self.values['-eTab_in_fees-'], 'int')
        rent = self.test(self.values['-eTab_in_rent-'], 'int')
        checkbonus = self.test(self.values['-eTab_in_checkbonus-'], 'int')
        checkfee = self.test(self.values['-eTab_in_checkfee-'], 'int')
        cashbonus = self.test(self.values['-eTab_in_cashbonus-'], 'int')
        cashfee = self.test(self.values['-eTab_in_cashfee-'], 'int')
        boothcustom = self.test(self.values['-eTab_in_boothcustom-'], 'int')
        boothcustomflag = self.test(self.values['-eTab_cb_booth-'], 'bool')
        commission, check, boothrent, comspec, checkdeal, checkoriginal, cashrate = (0 for i in range(1, 8))
        role = self.values['-eTab_om_role-']
        regType = True if role in ['Regular', 'Janitor', 'Owner'] else False
        if regType:
            self.togglePaygrade(role)
            commission = self.test(self.values['-eTab_c_commission-'], 'float')
            check = self.test(self.values['-eTab_c_check-'], 'float')
        else:
            self.togglePaygrade(role)
            comspec = self.test(self.values['-eTab_c_commissionspecial-'], 'float')
            checkdeal = self.test(self.values['-eTab_c_checkdeal-'], 'float')
            checkoriginal = self.test(self.values['-eTab_c_checkoriginal-'], 'float')
            cashrate = self.test(self.values['-eTab_c_cashrate-'], 'float')
        printchecks = self.values['-eTab_cb_printchecks-']
        # validating paygrades and amounts correspond
        if regType:
            checkpoint['paygrade'] = True
        if role:
            role.lower()
            checkpoint['paygrade'] = True

        if all(checkpoint.values()):  # if all data entries are valid or 'True'
            name = self.values['-eTab_in_empName-']
            # nameCapitalized is formally capitalize every word for esthetics in json
            nameCapitalized = ' '.join(word.capitalize() for word in self.values['-eTab_in_empName-'].split())
            string.capwords(self.values['-eTab_in_empName-'])
            newEmp = {
                name: {'active': status,
                       'name': nameCapitalized.replace('  ', ' '), 'checkName': checkName.replace('  ', ' '),
                       'salonName': self.values['-eTab_om_salon2-'],
                       'pay6': pay6, 'pay7': pay7,
                       'fees': fees, 'rent': rent, 'printchecks': printchecks,
                       'checkbonus': checkbonus, 'checkfee': checkfee,
                       'cashbonus': cashbonus, 'cashfee': cashfee,
                       'boothcustom': boothcustom, 'boothcustomflag': boothcustomflag,
                       'type': {'role': role,
                                'regular': {'commission': commission, 'check': check, 'boothrent': boothrent},
                                'special': {'commissionspecial': comspec, 'checkdeal': checkdeal,
                                            'checkoriginal': checkoriginal, 'cashrate': cashrate}
                                },
                       'workdays': {
                                    'mon': self.values['-eTab_janitor_mon-'], 'tue': self.values['-eTab_janitor_tue-'],
                                    'wed': self.values['-eTab_janitor_wed-'], 'thu': self.values['-eTab_janitor_thu-'],
                                    'fri': self.values['-eTab_janitor_fri-'], 'sat': self.values['-eTab_janitor_sat-'],
                                    'sun': self.values['-eTab_janitor_sun-']
                                    }
                       }
                      }
            return newEmp
        else:
            failed = []
            for point, val in checkpoint.items():
                if not val:
                    failed.append('Entry: {}'.format(point))
            sg.popup_ok('Invalid Entry\n{}'.format(failed))

    def printDict(self, result, location):
        nicerResult = ''
        for key, value in result.items():
            if isinstance(value, dict):
                for subkey, subval in value.items():
                    if isinstance(subval, dict):
                        for sskey, ssval in subval.items():
                            nicerResult += '    {}: {}\n'.format(sskey, ssval)
                    else:
                        nicerResult += '  {}: {}\n'.format(subkey, subval)
            else:
                nicerResult += '{}: {}\n'.format(key, value)
        self.gui[location].update(nicerResult)

    def refreshETabList(self, salonName):
        # refresh list with employee removed
        show_active_only = self.gui['-eTab_c_status-'].get()
        self.eTab_lb_Emps[salonName] = self.ai.populateEmpList(salonName, show_active_only)
        self.gui['-eTab_lb-'].update(values=self.eTab_lb_Emps[salonName].keys())

    def refreshMTabList(self, salonName):
        if self.eTab_lb_Emps:
            self.gui['-mTab_lb-'].update(self.mTab_lb_Emps[salonName].keys())

    def refreshSalonLists(self):
        salonLists = ['-mTab_c_salon-', '-eTab_om_salon-', '-sTab_c_salon-', ]
        for l in salonLists:
            self.gui[l].update(values=self.activeSalonNames)

    def splitRange(self, sDate, eDate):
        """
            When using the webscrape sales function, we use this helper function to split the
            date range request if it wants a range of over 6 months. The reason for that is because
            if the range is to spread out, zota website might either take way too long to load the request
            or not at all (sometimes). So to avoid time out, we can request multiple parts to be more
            efficient
        Args:
            sDate: (string) mm/dd/yyyy
            eDate:

        Returns:
            (list) list of date ranges, that are converted back to string format
        """
        sday = datetime.datetime.strptime(sDate, '%m/%d/%Y')
        eday = datetime.datetime.strptime(eDate, '%m/%d/%Y')
        dif = eday - sday
        if dif > datetime.timedelta(days=180):
            firsthalf = sday + (dif / 2)
            secondhalf = firsthalf + datetime.timedelta(days=1)
            firsthalf = datetime.datetime.strftime(firsthalf, '%m/%d/%Y')
            secondhalf = datetime.datetime.strftime(secondhalf, '%m/%d/%Y')
            return [[sDate, firsthalf], [secondhalf, eDate]]
        else:
            return [[sDate, eDate]]

    def status(self, msg):
        self.gui['-notice-'].update(msg)

    def togglePaygrade(self, role):
        rkeys = ['-eTab_c_commission-', '-eTab_c_check-']
        skeys = ['-eTab_c_commissionspecial-', '-eTab_c_checkdeal-', '-eTab_c_checkoriginal-', '-eTab_c_cashrate-']
        paytype = ''
        if role in ['Cash', 'Checkdeal']:
            [self.gui[i].update(disabled=True) for i in rkeys]
            [self.gui[i].update(disabled=False) for i in skeys]
        elif role in ['Regular', 'Janitor', 'Owner']:
            [self.gui[i].update(disabled=False) for i in rkeys]
            [self.gui[i].update(disabled=True) for i in skeys]
        else:
            # disable all if role not chosen
            [self.gui[i].update(disabled=False) for i in rkeys]
            [self.gui[i].update(disabled=False) for i in skeys]

    def updateValues(self, values):
        self.values = self.values | values

    def verifyDates(self, sdate=None, edate=None):
        """
            takes two dates and make sure it is sdate is before edate.
            and if the "this week" checkbox is marked, figure out the monday
            and sunday dates automatically if current day is mid-week. This
            method sets global variables instead of returning any value.
            The other part of the test will convert text to datetime and compare
            dates.
        Args:
            sdate: string mm/dd/yyyy
            edate:

        Returns:

        """
        if self.values['-mTab_cb_thisweek-']:
            cday = datetime.date.today()
            daynum = cday.weekday()
            startday = cday - datetime.timedelta(daynum)
            endday = startday + datetime.timedelta(6)
            self.startDate = startday.strftime('%m/%d/%Y')
            self.endDate = endday.strftime('%m/%d/%Y')
            self.gui['-main_in_sDate-'].update(self.startDate)
            self.gui['-main_in_eDate-'].update(self.endDate)
            self.dates = True
        else:
            sd = ''
            ed = ''
            if sdate and edate:
                try:
                    sd = datetime.datetime.strptime(sdate, '%m/%d/%Y')
                    ed = datetime.datetime.strptime(edate, '%m/%d/%Y')
                except Exception:
                    return
            else:
                try:
                    sd = datetime.datetime.strptime(self.values['-main_in_sDate-'], '%m/%d/%Y')
                    ed = datetime.datetime.strptime(self.values['-main_in_eDate-'], '%m/%d/%Y')
                except Exception:
                    return
            if sd <= ed:
                self.dates = True
                self.startDate = sd.strftime('%m/%d/%Y')
                self.endDate = ed.strftime('%m/%d/%Y')
            else:
                self.dates = False

    def getMarker(self, values):
        salon_marker = []
        if values == 'All':
            salon_marker = [True for i in self.activeSalonNames]
        else:
            for item in self.activeSalonNames:
                salon_marker.append(True if values == item else False)
        return salon_marker

    def test(self, expression, dataType):
        if dataType == 'int':
            try:
                return int(expression)
            except Exception as e:
                return 0
        elif dataType == 'float':
            try:
                return float(expression)
            except Exception as e:
                return float(0)
        elif dataType == 'bool':
            try:
                return bool(expression)
            except Exception as e:
                return False
        elif dataType == 'date':
            try:
                datetime.datetime.strptime(expression, '%m/%d/%Y')
                return expression
            except Exception:
                return False
        else:
            return False

class BtnInfo:
    def __init__(self, state=True):
        self.state = state  # can be True, False, or None (disabled)

    def setState(self, state):
        self.state = state


if __name__ == '__main__':
    toggle_btn_off = b'iVBORw0KGgoAAAANSUhEUgAAAGQAAAAoCAYAAAAIeF9DAAAPpElEQVRoge1b63MUVRY' \
                     b'//Zo3eQHyMBEU5LVYpbxdKosQIbAqoFBraclatZ922Q9bW5b/gvpBa10' \
                     b'+6K6WftFyxSpfaAmCEUIEFRTRAkQFFQkkJJghmcm8uqd763e6b+dOZyYJktoiskeb9OP2ne7zu' \
                     b''
    toggle_btn_on = b'iVBORw0KGgoAAAANSUhEUgAAAGQAAAAoCAYAAAAIeF9DAAARfUlEQVRoge1bCZRVxZn+qure' \
                    b''

    start = View()

'''
ways to print:
    regular:     print()
    gui popup:   sg.easy_print(msg)
                 sg.Print()
'''

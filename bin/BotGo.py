...
class BotGo:
    ...
    def dlEmpSales(self, salonName, id, uname, password, startDate, endDate):
        """
        ...
        self.connect(dlDir, id, uname, password)
    ...
    def connect(self, dlDir, id, uName, password):
        """
        ...
        self.driver.get("https://dashboard.gocheckin.net/login")
        # self.driver.maximize_window()
        usernameInput = self.driver.find_element(By.ID,"form_item_id")
        usernameInput.clear()
        usernameInput.send_keys(id)
        passInput = self.driver.find_element(By.ID,"form_item_username")
        passInput.clear()
        passInput.send_keys(uName)
        passInput = self.driver.find_element(By.ID, "form_item_password")
        passInput.clear()
        passInput.send_keys(password)
        # loginBtn = self.driver.find_element(By.CLASS_NAME,"css-litb8c ant-btn ant-btn-primary ant-btn-block")
        # loginBtn.click()
        self.driver.find_element(By.XPATH, '//*[@id="app"]/main/section/div[1]/div/form/div[5]/div/div/div/div/button').click()

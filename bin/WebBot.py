...
class WebBot:
    ...
    def getPayroll(self, startDay, endDay):
        ...
        helper.waitLoadingClick(s, 5, 'startDate input', self.driver)
        ...
        for a in self.activeEmp:
            ...
            helper.waitLoadingClick(reportBtn, 5, "Report Button",
    ...
    def exportParToXlsx(self, fname):
        ...
        self.sheet = self.parsedData[1][3].replace('/', '.')
        # check to see if .xlsx exists or create one
        if os.path.isfile(fname):
            # if book exists, now check for sheet exists
            book = openpyxl.load_workbook(filename=fname, )  # load book
            ...
            self.sheet = book.create_sheet(self.sheet)
            for employee in self.parsedData:
                for lines in employee:
                    self.sheet.append(lines)
            book.save(fname)
